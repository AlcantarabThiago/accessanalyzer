...
const Result = () => {
  ...
  useEffect(() => {
    ...
    if (image) {
      ...
      // Se nenhuma imagem foi passada, você pode redirecionar ou mostrar um estado de erro
      // navigate('/'); // Opcional: Voltar para a tela inicial se não houver imagem
    ...
    if (propErro) {
      ...
      // Se não há erro nem mensagem da IA (talvez a análise foi neutra ou mockada)
      // Aqui você pode adicionar as sugestões mockadas como fallback se quiser
      // Por enquanto, vamos deixá-las vazias se não houver mensagem real.
      setSuggestions([]);
      setErroDisplay("Análise concluída, mas sem observações específicas da IA.");
    }

    // Remova a chamada à API do n8n daqui. Ela deve estar na Analysis.tsx
    // Remova também o navigate("/result", ...) daqui, pois esta já é a tela de resultado.
  ...
  const getSeverityIcon = (severity: string) => {
    switch (severity) {
      ...
      case 'média': return <Eye className="w-4 h-4 text-yellow-500" />; // Usando Eye para média, como no seu original
  ...
  if (!image && !erroDisplay) { // Só redireciona se não houver nem imagem nem erro para exibir
    navigate('/');
    return null;
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 via-white to-green-50">
      <header className="border-b bg-white/80 backdrop-blur-sm">
        <div className="container mx-auto px-4 py-4">
          <div className="flex items-center gap-4">
            <Button
              variant="ghost"
              onClick={() => navigate('/analysis', { state: { image: image } })} /* Volta para analysis, passando a imagem novamente */
      ...
      <main className="container mx-auto px-4 py-6">
        <div className="grid lg:grid-cols-3 gap-6 h-[calc(100vh-140px)]">
          ...
          {/* Coluna para a imagem e talvez "outros uploads" (removido a simulação de outros uploads para simplificar) */}
          <Card className="lg:col-span-2"> {/* Aumentado para 2 colunas para focar na imagem e sugestões */}
            <CardHeader>
          ...
          <Card className="lg:col-span-1">
            ...
            <CardContent className="h-[calc(100%-80px)] p-0">
              <ScrollArea className="h-full px-6">
                <div className="space-y-6 py-6"> {/* Adicionado padding vertical */}
                  {erroDisplay && (
                        ...
                        <div className="flex items-start gap-3">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="bg-gray-50 border rounded-lg p-3 mb-3">
                              ...
                              <p className="text-sm text-gray-600 whitespace-pre-wrap">{suggestion.description}</p> {/* Adicionado whitespace-pre-wrap */}
                            </div>
                            <div className="bg-green-50 border border-green-200 rounded-lg p-3">
                              <h5 className="font-medium text-green-900 mb-1">Ação recomendada:</h5>
                              <p className="text-sm text-green-800 whitespace-pre-wrap">{suggestion.improvement}</p> {/* Adicionado whitespace-pre-wrap */}
                            </div>
                            {suggestion.position && (
          ...
          {/* Removido "Resultado Final Otimizado" e "Uploads do Usuário" em separado para simplificar o layout e focar na imagem e sugestões */}
